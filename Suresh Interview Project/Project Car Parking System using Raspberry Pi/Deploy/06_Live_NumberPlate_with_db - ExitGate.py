...
def car_registry(text:str) -> str:
    ...
    cursor.execute(f"""SELECT * FROM parking where number_plate='{text}' and in_out_status = 'IN'; """)
    # Fetching the data
    ...
    if len(data) >= 1:
        ...
        # Retrieving data from the table
        cursor.execute(f"""SELECT * FROM parking where number_plate='{text}' and in_out_status = 'OUT'; """)
        # Fetching the data
