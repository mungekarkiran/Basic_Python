...
def builMultiLabelModels(X_train, y_train, classifierModel):
    ...
    logging.info(f"'Accuracy': {Accuracy_score}, 'Loss': {Loss}, 'Hamming_loss': {Hamming_loss}")
...
def classification_report_ensemble_voting_classifier(y_train, y_pred_new):
    ...
    return (Accuracy_score, Loss, Hamming_loss)
...
def fit_ensemble_voting_classifier(X_train, y_train, classifiers):
    ...
    try:
        y_pred_new = predict_ensemble_voting_classifier(X_train, RESULT_FILE_PATH)

        Accuracy_score, Loss, Hamming_loss = classification_report_ensemble_voting_classifier(y_train, y_pred_new)
