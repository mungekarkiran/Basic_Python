...
try:
    # create stud_result table
    cursor.execute(f'CREATE TABLE IF NOT EXISTS stud_result (id TEXT PRIMARY KEY, full_name TEXT NOT NULL, email_id TEXT NOT NULL UNIQUE, phone_number TEXT NOT NULL, input TEXT NOT NULL, scores TEXT NOT NULL, result TEXT NOT NULL) ')
    connection.commit()
except Exception as e: 
    print('Table stud_result is NOT created : \n',e)
...
def qanda():
    if request.method == 'POST':
        ...
        scores = str(list_of_score)

        cursor.execute(f'''INSERT INTO stud_result (id, full_name, email_id, phone_number, input, scores, result) VALUES ("{u_id}", "{full_name}", "{email_id}", "{phone_number}", "{input}", "{scores}", "{result}" ); ''')
        connection.commit()
...
def jobtitle():
    ...
    if request.method == 'POST':
        ...
        c_f = request.files['certificate_file']
        
        u_id = uuid.uuid1()
        directory = u_id.hex
        # Parent Directories
        parent_dir = "static//style//csv"
        # Path
        my_path = os.path.join(parent_dir, directory)
        # Create the directory
        os.makedirs(my_path)
        
        file_path = os.path.join('static//style//csv', secure_filename(f.filename)) 
        c_file_path = os.path.join(my_path, secure_filename(c_f.filename)) 
        # static//style//csv
        # csv
        # uploads
        # Save the file to ./uploads
        f.save(file_path)
        c_f.save(c_file_path)
